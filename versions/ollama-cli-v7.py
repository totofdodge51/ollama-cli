#!/usr/bin/env python3
"""
Ollama CLI v7 - Assistant LLM interactif amélioré avec création de projet multi-fichiers.
"""
...
class OllamaCLI:
    ...
    def _get_header_panel(self):
        ...
        return Panel(Text(ASCII_LOGO, style=self.theme["logo"], justify="center"), title="Ollama CLI v7", subtitle=subtitle, border_style=self.theme["header_border"])
    ...
    def handle_config_command(self):
        ...
        config_panel = Panel(f"Lanceur de terminal: `[cyan]{self.terminal_launcher}[/]`\nAccès Web: {current_web}", title="Configuration Actuelle", border_style=self.theme["info_panel_border"])
    ...
    def _get_help_content(self):
        help_text = """# Aide de Ollama CLI v7
- `/quit, /exit, /q`: Quitter.
...
- `/project [list|save|load]`: Gérer les projets.
    ...
    def chat_loop(self):
        ...
        while True:
            try:
                ...
                with console.status(f"[bold {self.theme['warning']}]L'assistant réfléchit...[/bold {self.theme['warning']}]"):
...
def main():
    parser = argparse.ArgumentParser(description="Ollama CLI v7 - Assistant de code interactif avec création de projet.")
