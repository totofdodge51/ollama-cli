#!/usr/bin/env python3
"""
Ollama CLI v10 - Assistant LLM interactif amélioré avec création de projet multi-fichiers.
"""
...
class OllamaCLI:
    ...
    def _get_header_panel(self):
        ...
        return Panel(Text(ASCII_LOGO, style=self.theme["logo"], justify="center"), title="Ollama CLI v10", subtitle=subtitle, border_style=self.theme["header_border"])
...
"""
    ...
    def _get_help_content(self):
        help_text = """# Aide de Ollama CLI v10
- `/quit, /exit, /q`: Quitter.
...
"""
    ...
    def process_response(self, response: str):
        ...
        # Toujours ajouter la réponse de l'assistant à l'affichage pour le débogage.
        ...
        # Maintenant, vérifier l'utilisation d'outils dans la réponse et agir en conséquence.
        if '<project_creation>' in response:
            ...
            pass  # La modification de secours a été gérée
    ...
    def handle_fallback_modification(self, response: str) -> bool:
        # Condition 1: A code block exists
        code_match = re.search(r'```(\w*)?\n?(.*?)```', response, re.DOTALL)
        if not code_match:
            return False

        # Condition 2: At least one file is loaded
        if not self.loaded_files:
            return False

        # lang = (code_match.group(1) or "").lower() # No longer needed
        new_content = code_match.group(2).strip()
        ...
        if len(self.loaded_files) == 1:
            ...
            # More than one file, ask the user
            ...
            try:
                choice = Prompt.ask(
                    "\nSélectionnez le fichier à modifier (ou pressez Entrée pour annuler)",
        ...
        # The language check has been removed to prevent false negatives.
        # User validation of the diff is now the primary safety mechanism.
        ...
        if Confirm.ask(f"\n[bold]Appliquer cette modification au fichier {path_to_modify} ?[/bold]"):
            ...
            console.print(f"[{self.theme['success']}]✓ {msg}[/{self.theme['success']}]" if success else f"[{self.theme['error']}]✗ {msg}[/{self.theme['error']}]")
            if success:
    ...
    def handle_project_creation(self, response: str):
        ...
        if Confirm.ask(f"\n[bold]Créer ces {len(files)} fichier(s) ?[/bold]"):
            ...
            for path, file_content in files:
                ...
                # Nettoyer le contenu du fichier pour retirer les balises Markdown
    ...
    def handle_file_modifications(self, response: str):
        ...
        # Stocker le contenu nettoyé pour l'écriture de fichier plus tard
        ...
        for path, new_content_raw in files_to_modify:
            ...
            if original_content is None:
                ...
                if not success:
                    # Si le fichier n'existe pas, on le traite comme une création
                    original_content = ""
                else:
